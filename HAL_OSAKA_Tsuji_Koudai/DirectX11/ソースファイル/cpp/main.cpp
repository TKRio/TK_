...
//-----------------------------------------------------------------------------
#define		NAME			"win32A"
#define		TITLE			"DirectX11 MeshCutデモ"
						...
						LPARAM lParam)	
{
	...
	case WM_KEYDOWN:
		...
		break;
	case WM_LBUTTONDOWN:
		Mouse::GetInstance()->SetLButtonClick(TRUE);
		break;
	case WM_LBUTTONUP:
		Mouse::GetInstance()->SetLButtonClick(FALSE);
		break;
	case WM_RBUTTONDOWN:
	{
		POINT pt;
		...
		pt.x = pt.x - SCREEN_X / 2;
		pt.y = pt.y - SCREEN_Y / 2;
		Mouse::GetInstance()->SetPointerPos(pt.x, pt.y);
		Mouse::GetInstance()->SetRButtonClick(TRUE);
	}
	break;
	case WM_RBUTTONUP:
		Mouse::GetInstance()->SetRButtonClick(FALSE);
		break;
	case WM_MBUTTONDOWN:
		Mouse::GetInstance()->SetWButtonClick(TRUE);
		break;
	case WM_MBUTTONUP:
		Mouse::GetInstance()->SetWButtonClick(FALSE);
		break;
	case WM_MOUSEWHEEL:
	{
		...
		{
			// ↑に回転（チルト）した
			Mouse::GetInstance()->SetWheelUpDown(1);
		}
		else if (nNotch < 0)
		{
			// ↓に回転（チルト）した
